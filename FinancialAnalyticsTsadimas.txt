##################################################################
#                                                                #
# purpose   : Financial Analytics Final Assignment               #
#                                                                #
...
setwd("C:/MsC/Financial Analytics/Assignment_June_2024")
...
# part A
# Read the Excel file
file_path <- "C:/MsC/Financial Analytics/Assignment_June_2024/US_FUND_DATA.xlsx"
data <- read_excel(file_path)

# Rename the first column to "Date" if it contains date information
names(data)[1] <- "Date"
data$Date <- as.Date(data$Date)  # Convert the 'Date' column to Date format, adjust the format if needed

# Print the total number of columns and the first few entries in the "Date" column to verify
print(paste("Total number of columns:", ncol(data)))

# Select columns from 121 to 210 for analysis
my_funds_returns <- data[, 122:211]
colnames(my_funds_returns)  # This will print the names of the columns to verify you have the right ones

# Display the new data frame
print(my_funds_returns)

# Number of rows in the returns data
num_rows <- nrow(my_funds_returns)

# Generate a sequence of dates for the exact number of rows in the data
Time <- seq(as.Date("1963-07-01"), by="months", length.out = num_rows)

# Function to get last day of the month using lubridate
last_day_of_month <- function(date) {
  ceiling_date(date, "month") - days(1)
}

# Apply the function to get the last day of each month
Time <- sapply(Time, last_day_of_month)
Time <- as.Date(Time)

# Convert the data to a timeSeries object
My_Funds_Returns <- timeSeries(my_funds_returns, Time)

# Verify the time series object
print(head(My_Funds_Returns))

# Define the estimation and out-of-sample periods
initial_estimation_period <- My_Funds_Returns[1:624, ]  # From 7/1963 to 7/2015 (52 years * 12 months)
out_of_sample_period <- My_Funds_Returns[625:672, ]     # From 8/2015 to 7/2019 (4 years * 12 months)

# Print to verify
print(head(initial_estimation_period))
print(head(out_of_sample_period))
...
# Convert the 'Date' column to a proper Date format and adjust to the last day of the month
factors$Date <- as.Date(paste0(factors$Date, "01"), format = "%Y%m%d")
factors$Date <- ceiling_date(factors$Date, "month") - days(1)

# Print the head of the factors data to verify
print(head(factors))

# Verify the number of rows in the factor data
num_factor_rows <- nrow(factors)
num_returns_rows <- nrow(my_funds_returns)

# Extend factor data to match the number of rows in returns data
if (num_factor_rows < num_returns_rows) {
  additional_rows <- num_returns_rows - num_factor_rows
  last_row <- factors[nrow(factors), -1]  # Exclude the Date column
  
  # Generate new dates starting from the last date in the factors
  new_dates <- seq(as.Date(factors$Date[num_factor_rows]), by = "months", length.out = additional_rows + 1)[-1]
  new_dates <- ceiling_date(new_dates, "month") - days(1)
  
  # Create the extension dataframe
  extension <- as.data.frame(matrix(rep(as.numeric(last_row), additional_rows), nrow = additional_rows, byrow = TRUE))
  extension$Date <- new_dates
  colnames(extension)[1:ncol(extension)-1] <- colnames(factors)[-1]  # Preserve column names except Date
  
  # Combine original factors with the extension
  factors <- rbind(factors, extension)
}

# Verify the extended factors data
print(nrow(factors))  # Should match num_returns_rows
print(head(factors))  # Check the extended data
print(tail(factors))  # Check the extended data at the end

# Ensure the Date column is at the correct position
Time <- factors$Date
factors <- factors[, -1]
...
# Part B, Question b) - Construct optimal portfolios based on the Single Index Model
...
# Part B, Question c) - Construct optimal portfolios based on multivariate multiple regression models
...
# Alternative modeling approach: Fama-French 5-Factor Model
